...
void LCD_drawBuffer(const unsigned char bitmap[], const uint8_t bitmapSize, const uint8_t bitmap_width, uint8_t initialX, uint8_t initialY) {

	uint8_t line = 0;
	uint8_t posX = initialX;
	uint8_t posY = initialY;

	uint8_t initialYRemainder = (initialY % 8);

	const uint8_t sizeOfFirstEl = sizeof(bitmap[0]);
	const uint8_t bitmapByteSize = bitmap_width / sizeOfFirstEl;

	for (uint8_t i = 0; i < bitmapSize; i++) {

		posX = initialX + i - (line * bitmap_width);
		posY = (initialY - initialYRemainder) + line;

		lcd.buffer[posX + (posY) * LCD_WIDTH] = bitmap[i];

		line = (i + 1 - ((i + 1) % bitmapByteSize)) / bitmapByteSize;
	}
}
 ...
 */
void shift_right(unsigned char *ar, const uint8_t size, uint8_t shift)
{
	if(shift == 0)
	{
		return;
...
void LCD_setBuffer(uint8_t bufferAdress, uint8_t val)
{
	lcd.buffer[bufferAdress] = val;
}
